...
This patch release addresses a critical issue with the new v3 Over-The-Air (OTA) update system where the application would fail to install downloaded updates and remain stuck in the background.

---

### ✨ Highlights

* Fixed OTA updater installation freeze
* Restored native Windows UAC handling for updates

---

### Fixes

* Fixed a deadlock in the OTA update process where the application would minimize to the system tray instead of closing completely, causing the NSIS installer to hang indefinitely in the background.
* Reverted to the native `electron-updater` installation flow (`quitAndInstall`), ensuring proper administrator elevation prompts (UAC) are triggered on Windows without silent failures.
...
В этом патче исправлена критическая проблема с новой системой OTA-обновлений v3, из-за которой приложение не могло установить скачанные обновления и зависало в фоновом режиме.

---

### ✨ Основные особенности

* Исправлено зависание при установке OTA-обновлений
* Восстановлена стандартная обработка запросов UAC (Windows) при обновлении

---

### Исправления

* Исправлена взаимная блокировка (deadlock) в процессе OTA-обновления, когда приложение сворачивалось в системный трей вместо полного закрытия, из-за чего установщик NSIS бесконечно зависал в фоновом режиме.
* Возвращен стандартный процесс установки `electron-updater` (`quitAndInstall`), что гарантирует корректное появление запросов на повышение прав (UAC) в Windows без «тихих» сбоев.
